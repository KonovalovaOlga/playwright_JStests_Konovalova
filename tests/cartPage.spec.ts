...
test.only("Adding item to the cart and check its correct price on Review and Payments", async ({
  page,
}) => {
  ...
  //.counter-number
  await expect(page.locator(".counter-number")).toBeVisible();
  ...
  const shippingPriceLocator = page
    .locator(
      'span.price[data-bind="text: getFormattedPrice(method.price_excl_tax)"]'
    )
    .first();
  ...
  await page.waitForTimeout(5000);
  await page
    .locator('button[type="submit"].button.action.continue.primary')
    .click();
  //Taking the product price, shipping cost and the final price from the final box on the payment page
  //for comparison
  const cartSubtotalLocator = page.locator(".opc-block-summary .sub .price");
  await expect(cartSubtotalLocator).toBeVisible();
